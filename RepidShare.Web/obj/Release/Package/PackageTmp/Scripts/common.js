...
function bindSort(obj, sortByID, sortOrderID, actionTypeID, fnCallback,  PagePrefix) {
    ...
    if (sortby == previouseSortBy) {
        if (sortOrder == '0') {
            $("#" + PagePrefix + sortOrderID).val("1");
            $(obj).addClass("desc");
        }
        else {
            $("#" + PagePrefix + sortOrderID).val("0");
            $(obj).addClass("asc");
        }
...
function createMultipleFooter(footerContainer, PagePrefix, totalPageID, PageSizeID, ActionTypeID, CurrentPageID, fnCallback) {

    
    createFooter(footerContainer, parseInt($("#" + PagePrefix + totalPageID).val()), parseInt($("#" + PagePrefix + PageSizeID).val()), (PagePrefix + ActionTypeID), (PagePrefix + CurrentPageID), fnCallback,PagePrefix)
}

function createFooter(footerContainer, totalPages, rowsPerPage, ActionTypeID, CurrentPageID, fnCallback, PagePrefix) {
   ...
   if (totalPages > 1) {
        ...
        if (parseInt(currentPage) > 1) {
            footer = footer + "<li class='prev-btn'><a href='javascript:void(0)' title='previous' class='page " + PagePrefix + "' id=" + (parseInt(currentPage) - 1) + "></a></li>"
        }
       ...
       // for (var i = 1; i < (parseInt(totalPages) + 1); i++) {
        ...
        if (parseInt(currentPage) <parseInt(totalPages)) {
            footer = footer + "<li class='next-btn'><a href='javascript:void(0)' title='next' class='page " + PagePrefix + "' id=" + (parseInt(currentPage) + 1) + "></a></li>"
        }
        footer=footer+"</ul>"
        footer = footer;
...
function showSpinnerForDecimal(id, minvalue, maxvalue) {
 ...
 function TrimPreFixZeroforDecimal(obj) 
 {
     if (obj.value == '')
         return obj.value;
     else
 ...
 function OpenEditDialogMedium(url, titleText, dialogID) {
        $('#light-box').removeClass("model-full");
        openEditDialog(url, titleText, dialogID, false);
        $('#light-box').addClass("model-medium");       
    }


    function OpenEditDialogFull(url, titleText, dialogID) {
        $('#light-box').removeClass("model-medium");
        openEditDialog(url, titleText, dialogID, false);
        $('#light-box').addClass("model-full");
